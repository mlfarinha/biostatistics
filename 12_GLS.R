############################################################################
###### 12. SCRIPT TO STUDY INFLUENCE OF COVID ON HYPERTENSION PROGRAM ######
############################################################################

# ----------------------------------------------------
# 1. Import the necessary data
# 2. Build the dataset
# 3. Fitting model with relevant predictors GLS
#
# Miguel Farinha 
...
arima.fit <- auto.arima(F_all$residuals, seasonal = F, approximation = F, stepwise = F); arima.fit # ARIMA(2,0,1)

rm("arima.M0", "arima.M1", "arima.M5", "arima.M6", "arima.M7", "arima.I1", "arima.I2", "arima.I3", "arima.I4",
   "arima.fit")

# Experimenting using AR(p) or MA(q)
F_all.GLS.AR2 <- gls(meanBP ~ med_school_closing + med_workplace_closing + med_stay_home_restrictions +
                       med_internal_movement_restrictions + mean_stringency_index + meanCSP + meanCSPpresent +
                       meanCSPabsent + meanCSPhome + ARSf, data = total, correlation=corARMA(p=2), method="ML")
...
F_all.GLS.AR4 <- gls(meanBP ~ med_school_closing + med_workplace_closing + med_stay_home_restrictions +
                       med_internal_movement_restrictions + mean_stringency_index + meanCSP + meanCSPpresent +
                       meanCSPabsent + meanCSPhome + ARSf, data = total, correlation=corARMA(p=4), method="ML")
summary(F_all.GLS.AR4) # AIC, BIC, logLik = 944.674 982.3722 -454.337

F_all.GLS.MA1 <- gls(meanBP ~ med_school_closing + med_workplace_closing + med_stay_home_restrictions +
                       med_internal_movement_restrictions + mean_stringency_index + meanCSP + meanCSPpresent +
                       meanCSPabsent + meanCSPhome + ARSf, data = total, correlation=corARMA(q=1), method="ML")
...
F_all.GLS.ARMA <- gls(meanBP ~ med_school_closing + med_workplace_closing + med_stay_home_restrictions +
                       med_internal_movement_restrictions + mean_stringency_index + meanCSP + meanCSPpresent +
                       meanCSPabsent + meanCSPhome + ARSf, data = total, correlation=corARMA(p=2,q=1), method="ML")
...
anova(F_all.GLS.ARMA, F_all.GLS.ARMA2) # ARMA(2,0,1) vs ARMA(4,0,2) -> p = 0.0058
...
F_all.GLS <- gls(meanBP ~ med_school_closing + med_workplace_closing + med_stay_home_restrictions +
                   med_internal_movement_restrictions + mean_stringency_index + meanCSP + meanCSPpresent +
                   meanCSPabsent + meanCSPhome + ARSf,
...
color1 <- c("#E41A1C", "#377EB8","#4DAF4A","#984EA3","#FF7F00","#FFFF33","#A65628","#F781BF","#999999", "black")
